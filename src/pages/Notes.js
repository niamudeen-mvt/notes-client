...
import { addNotes, deleteNotesById, editNotesById, getNotes } from "../services/api/notes";
...
const NotesPage = () => {
  ...
  return (
    <section className="common_container flex_center">
                  ...
                  <Col className="flex_center mb-5" lg={3} >
                    <div className=" p-5 note_card">
                          ...
                          isEdit?.edit && isEdit.index === index ?
                            <>
                              ...
                              {note.message?.length > 150 ?
                                <span className="cursor" onClick={() => handleOpenModal(note.message, "MORE")}>...more</span>
                                : null}
                            </>
                            :
                            <p>{(el.message).substring(0, 200)}
                              {el.message?.length > 200 ?
                                <span className="cursor" onClick={() => handleOpenModal(el.message, "MORE")}>...more</span>
                                : null}
                            </p>
                      ...
                      <div className="icon_menu">  {
                        isEdit?.edit && isEdit.index === index ?
                          <FaCheck color="green" fontSize={'25px'} onClick={() => handleEditNote(el._id, index)} className="cursor" /> : null}
                        <BiSolidMessageSquareEdit color="#0d6efd" fontSize={'25px'} onClick={() => handleEdit(index)} className="cursor" />
                        <MdDelete color="red" fontSize={'25px'} onClick={() => handleDelteNote(el._id)} className="cursor" />
                      </div>
                    </div>
                  </Col>
                )
      ...
      <CustomModal showModal={showModal} setShowModal={setShowModal} modalHeading={contentType.key === "MORE" ? contentType.heading : "ADD NOTES"} handleSubmit={handleAddNotes} contentType={contentType.key}>
